# ライトノベルを書く

読み易く、クスリと笑えて時にはちょっとしたお色気もあるライトノベル。

## 執筆ルール
- 執筆前に必ず world.md / characters.md / plot.md / style.md /
  summaries/ 内の直前章の要約を読むこと
- 1セッションで書くのは1章のみ(6,000〜9,000字)。複数章を一気に書かない
- 章を書き終えたら:
  1. summaries/chNN.md に要約(登場人物・確定した設定・回収/新設した伏線)を書く
  2. continuity.md に新しく確定した設定事実を追記する
  3. gags.md に使用したネタを記録する(同型ギャグの再利用禁止)
- 執筆後、editor-checklist.md に従いセルフレビューし、指摘箇所を修正してから完了とする
